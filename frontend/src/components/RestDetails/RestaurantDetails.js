...
useEffect(() => {
  fetch(`https://zomato-clone-11.herokuapp.com/restaurant/details/${rName}`,{method:'GET'})
...
const fetchMenu=()=>{
  fetch(`https://zomato-clone-11.herokuapp.com/menu/${rName}`,{method:'GET'})
  .then(response=>response.json())
  .then(data=>{
  setMenu(data.data); })
}
 ...
 const openRazorpay=async()=>{
  ...
  try{
    ...
    let orderData;
    orderData = await fetch('https://zomato-clone-11.herokuapp.com/pay',{
    method:'POST',
  ...
  const options = {
    ...
    handler:function(response){
      ...
      fetch('https://zomato-clone-11.herokuapp.com/pay/save',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({
